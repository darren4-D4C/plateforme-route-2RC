...
function submitNew(event) {
    ...
    const quartier = document.getElementById('inputQuartier').value.trim();
    ...
    row.innerHTML = `
        ...
        <td class="user-matricule">${escapeHTML(quartier)}</td>
        <td class="user-matricule">${escapeHTML(topo)}</td>
        <td class="text-right">
...
function editRow(event, linkEl) {
    ...
    document.getElementById('inputQuartier').value = cells[1].textContent.trim();
    document.getElementById('inputTopo').value = cells[2].textContent.trim();

    const form = document.getElementById('newForm');
    form.onsubmit = (e) => {
        e.preventDefault();
        cells[0].textContent = document.getElementById('inputNom').value.trim();
        cells[1].textContent = document.getElementById('inputQuartier').value.trim();
        cells[2].textContent = document.getElementById('inputTopo').value;
...
function removeRow(event, linkEl) {
    event.preventDefault();
    if (!confirm('Retirer ce tronçon ?')) return;
